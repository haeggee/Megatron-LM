...
export MODEL=/iopsstor/scratch/cscs/dfan/copies/Megatron-LM-debug/logs/Meg-Runs/meta_data_conditioning_masking_out/apertus-1b-21n-4096sl-504gbsz-fw-edu-url_suffix_v2/checkpoints/
export NAME=URL-suffix-v2-5shots-$(date '+%Y-%m-%d_%H-%M-%S')
export ARGS="--convert-to-hf --size 1 --wandb-entity meta-robots --wandb-project meta_eval_nips_Apr26 --wandb-id $NAME --bs 32 --tokens-per-iter 2064384 --tasks scripts/evaluation/english_eval --lm-eval-branch ctx_5shot"
