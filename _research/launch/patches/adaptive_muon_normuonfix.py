...
class AdaptiveMuon(muon.Muon):
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if len(state) == 0:
                ...
                elif self.moment2_method in ("normuon", "normuonfix"):
    ...
    def _apply_moment2_normalization(
        ...
        if self.moment2_method == "adamuon":
        ...
        elif self.moment2_method == "normuonfix":
            # NorMuonFix (port of upstream c597a497): accumulate moment2 on the raw orthogonalized
            # gradient BEFORE muon scale_mode is applied, then re-apply scale after normalization.
            # In the buggy normuon path above, orth_grad already has scale_mode baked in by
            # Muon._get_update; the sqrt-division here effectively cancels that scale. By undoing
            ...
            moment2.lerp_(v_mean, 1 - beta2)
            step_size = moment2.clamp_min(eps).rsqrt_()
            return (raw_orth_grad * step_size) * scale_factor
