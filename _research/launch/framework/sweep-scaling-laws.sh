...
#   master:      all LRs width-independent, ~ 1/l^0.25
#
...
# ── grid ─────────────────────────────────────────────────────────────────────
# SIZES="270m-moe 420m-moe 810m-moe"
SIZES="270m-moe 420m-moe"
# tokens are set to be ~55 tok/active-param diagonal
# 270m@15, 420m@23.3, 810m@44.15
TOKENS="7.5"   # billions; 15 snaps to 3,584,000 samples (14.68B)
...
MBS=${MBS:-8}
if [ "$CLUSTER" = mi300 ]; then
    if [ "$SIZES" = "810m-moe" ] || [[ "$SIZES" =~ "810m-moe" ]]; then
        MBS=${MBS:-2}
    else
        MBS=${MBS:-4}
    fi
elif [ "$SIZES" = "810m-moe" ] || [[ "$SIZES" =~ "810m-moe" ]]; then
    MBS=${MBS:-4}
fi
