...
#
# Transformer++ MoE, ~0.44B active / ~2.97B total. 24L / 1024H / 16h / 4kv (GQA).
# Sparse MoE: 64 routed experts top-2 + 1 shared (half a routed expert's width),
# first layer dense then 23 MoE (~5% dense). moe_ffn = hidden/1.75. ~6.5%
...
# 15B tokens: 3,662,109 samples * 4096 seq = 15.0B, ~57k steps. FP8 blockwise.
#
...
# shared at half a routed expert's width; first layer dense, the other 23 MoE.
MOE_ARGS=(
    --num-experts 64
    --moe-router-topk 2
