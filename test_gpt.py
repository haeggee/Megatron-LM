...
from megatron.training import pretrain, load_two_models
...
def model_provider_1(pre_process=True, post_process=True) -> Union[GPTModel, megatron.legacy.model.GPTModel]:
    """Builds the model.
...
if __name__ == "__main__":
    ...
    model1, model2 = load_two_models(
        model_provider_1,
        model_provider,
        ModelType.encoder_or_decoder,
    )

    assert len(model1) == len(model2), "Model lists must be same length"
    ...
    for idx, (m1, m2) in enumerate(zip(model1, model2)):
        ...
        params1_keys = set(params1.keys())
        params2_keys = set(params2.keys())

        missing_in_2 = params1_keys - params2_keys
        extra_in_2 = params2_keys - params1_keys

        if missing_in_2:
            print_rank_0(f"[Model {idx}] Missing parameters in model2: {missing_in_2}")
            all_match = False
        if extra_in_2:
            print_rank_0(f"[Model {idx}] Extra parameters in model2: {extra_in_2}")
            all_match = False

        for name, p1 in params1.items():
            if name not in params2:
                print_rank_0(f"[Model {idx}] Parameter '{name}' missing in model2")
                all_match = False
                continue

            p2 = params2[name]
            print_rank_0(f"[Model {idx}] Comparing parameter: {name}")
    ...
    for layer_name in ['embedding.word_embeddings.weight', 'output_layer.weight']:
